...
/**
* Add custom css inside a module if exist
* Creates the <link  > tag that links a custom css of a module
* @access  public
* @param   string
* @return  string
*/
...
{
	function module_css($file_name='', $media="all"){
		$CI =& get_instance();

		if(!empty($file_name))
			$uri =  APPPATH."modules/".$CI->router->class."/_assets/css/".$file_name.".css";
		else
			$uri =  APPPATH."modules/".$CI->router->class."/_assets/css/".$CI->router->class.".css";

		if(file_exists($uri))
			return '<link rel="stylesheet" type="text/css" href="'.base_url() .$uri.'" media="' . $media . '">'."\n";
	}
}

/**
* Add custom js inside a module if exist
* Creates the <script  > tag that links a custom css of a module
* @access  public
* @param   string
* @return  string
*/
...
{
	function module_js($file_name='', $atts=array()){
		$CI =& get_instance();

		if(!empty($file_name))
			$uri =  APPPATH."modules/".$CI->router->class."/_assets/js/".$file_name.".js";
		else
			$uri =  APPPATH."modules/".$CI->router->class."/_assets/js/".$CI->router->class.".js";

		if(file_exists($uri)){
			$element = '<script type="text/javascript" src="'.base_url() .$uri.'"';
